...
// Fix: Double processing command by Nur
const processedMessages = new Set();
const MESSAGE_CACHE_TIME = 10000; // 10 seconds
...
function getRole(threadData, senderID) {
	...
	// Priority check: ownerBot > adminBot > thread admins
...
return async function (event, message) {
	...
	// Prevent double-processing of same message
	if (processedMessages.has(messageID)) return;
	processedMessages.add(messageID);
	setTimeout(() => processedMessages.delete(messageID), MESSAGE_CACHE_TIME);
	...
	// Skip word-effect or empty messages
	if ((!body || body.trim() === '') && (!event.attachments || event.attachments.length === 0)) {
		console.log("Skipping word effect or empty message:", event);
		return;
	}
