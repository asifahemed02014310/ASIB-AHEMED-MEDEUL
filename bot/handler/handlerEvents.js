...
async function onStart() {
	// —————————————— CHECK USE BOT —————————————— //
	// Fallback to messageReply or message text if body is missing
	let adjustedBody = body || event.messageReply?.body || event.message?.text || '';
	if (!adjustedBody.startsWith(prefix))
		return;
	const dateNow = Date.now();
	const args = adjustedBody.slice(prefix.length).trim().split(/ +/);
	// ————————————  CHECK HAS COMMAND ——————————— //
	let commandName = args.shift().toLowerCase();
	let command = GoatBot.commands.get(commandName) || GoatBot.commands.get(GoatBot.aliases.get(commandName));
	// ———————— CHECK ALIASES SET BY GROUP ———————— //
	...
	function removeCommandNameFromBody(body_, prefix_, commandName_) {
		if (arguments.length) {
			...
			return adjustedBody.replace(new RegExp(`^${prefix}(\\s+|)${commandName}`, "i"), "").trim();
		}
	}
	// —————  CHECK BANNED OR ONLY ADMIN BOX  ————— //
	if (isBannedOrOnlyAdmin(userData, threadData, senderID, threadID, isGroup, commandName, message, langCode))
