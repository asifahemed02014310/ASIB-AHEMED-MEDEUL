module.exports = {
	config: {
		name: "badwords",
		aliases: ["badword"],
		version: "1.5", // Updated version number
		author: "NTKhang",
		countDown: 5,
		role: 1,
		description: {
			vi: "Bật/tắt/thêm/xóa cảnh báo vi phạm từ thô tục, nếu thành viên vi phạm sẽ bị cảnh báo, lần 4 sẽ kick khỏi box chat",
			en: "Turn on/off/add/remove bad words warning, if any member use badword, they will be warned, after 3 warnings they will be kicked out of the group"
		},
		category: "admin",
		guide: {
	...
	langs: {
		vi: {
			...
			warned: "⚠️ | Từ cấm \"%1\" đã được phát hiện trong tin nhắn của bạn, nếu tiếp tục vi phạm bạn sẽ bị cảnh báo thêm (1/%2).",
			warned2: "⚠️ | Từ cấm \"%1\" đã được phát hiện trong tin nhắn của bạn, đây là lần cảnh báo thứ %2/%3.",
			warned3: "⚠️ | Từ cấm \"%1\" đã được phát hiện trong tin nhắn của bạn, đây là lần cảnh báo cuối cùng (%2/%3).",
			warned4: "⚠️ | Từ cấm \"%1\" đã được phát hiện trong tin nhắn của bạn, bạn đã vi phạm %2 lần và sẽ bị kick khỏi nhóm.",
		...
		en: {
			...
			badWordsList: "📑 | List of banned words in your group: %1",
			...
			warned: "⚠️ | Banned word \"%1\" has been detected in your message. This is warning #1/%2.",
			warned2: "⚠️ | Banned word \"%1\" has been detected in your message. This is warning #%2/%3.",
			warned3: "⚠️ | Banned word \"%1\" has been detected in your message. This is your FINAL warning (#%2/%3).",
			warned4: "⚠️ | Banned word \"%1\" has been detected in your message. You have violated %2 times and will be kicked from the group.",
	...
	onStart: async function ({ message, event, args, threadsData, usersData, role, getLang }) {
		if (!await threadsData.get(event.threadID, "data.badWords"))
			await threadsData.set(event.threadID, {
				words: [],
				violationUsers: {},
				maxWarnings: 3 // Setting maximum warnings to 3
			}, "data.badWords");

		const badWords = await threadsData.get(event.threadID, "data.badWords.words", []);
		const maxWarnings = await threadsData.get(event.threadID, "data.badWords.maxWarnings", 3);
	...
	onChat: async function ({ message, event, api, threadsData, prefix, getLang }) {
		...
		const maxWarnings = threadData.data.badWords?.maxWarnings || 3; // Default to 3 if not set

		for (const word of badWordList) {
			if (event.body.match(new RegExp(`\\b${word}\\b`, "gi"))) {
				const currentWarnings = violationUsers[event.senderID] || 0;
				
				if (currentWarnings < maxWarnings) {
					// Increment warnings
					violationUsers[event.senderID] = currentWarnings + 1;
					await threadsData.set(event.threadID, violationUsers, "data.badWords.violationUsers");
					
					// Send appropriate warning message based on current warning count
					if (currentWarnings === 0) {
						message.reply(getLang("warned", word, maxWarnings));
					} else if (currentWarnings === maxWarnings - 1) {
						message.reply(getLang("warned3", word, currentWarnings + 1, maxWarnings));
					} else {
						message.reply(getLang("warned2", word, currentWarnings + 1, maxWarnings));
					}
					return;
				}
				else {
					// User has reached maximum warnings, kick them
					await message.reply(getLang("warned4", word, currentWarnings + 1));
